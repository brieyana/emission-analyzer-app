...
const SingleStack = ({ engineId }) => {
  const { emissions, engines } = useUserStore();
  ...
  const traces = useMemo(() => {
    ...
    if (!prediction || !ratedThrust) return tracesByLevel;

    ["CO", "NOX"].forEach((pollutant) => {
      const label = `${pollutant}`;
